...
KUBERNETES_KUBECTL_VERSION = 1.28.8
...
define KUBERNETES_KUBECTL_BUILD_CMDS
	$(TARGET_MAKE_ENV) $(KUBERNETES_KUBECTL_MAKE_ENV) $(MAKE1) -C $(@D) all WHAT="cmd/kubectl" GOFLAGS="-trimpath -buildvcs=false" KUBE_BUILD_PLATFORMS="linux/$(GO_GOARCH)"
endef
...
define HOST_KUBERNETES_KUBECTL_BUILD_CMDS
	...
	$(HOST_MAKE_ENV) $(HOST_KUBERNETES_KUBECTL_MAKE_ENV) $(MAKE1) -C $(@D) all WHAT="cmd/kubectl" GOFLAGS="-trimpath -buildvcs=false" KUBE_BUILD_PLATFORMS="$$(go env GOOS)/$(go env GOARCH)" KUBE_CGO_OVERRIDES=kubectl
endef
